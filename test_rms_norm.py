...
def main():
    ...
    with profiler.profile(use_cuda=True, record_shapes=True) as prof:
        ...
        for last_dim in last_dimensions:
            ...
            x = torch.randn(input_shape, requires_grad=True).to("cuda")
            w = torch.randn(last_dim, requires_grad=False).to("cuda")
            bias = torch.randn(last_dim, requires_grad=False).to("cuda")
            dy = torch.randn(input_shape, requires_grad=False).to("cuda")
