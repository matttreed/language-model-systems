...
def train_model(version: str, type: str, world_size, sharded):
    ...
    rank = dist.get_rank()
    extra_info = "_sharded" if sharded else ""
    html_export_path = os.path.join(current_dir, f'benchmarking/memory_ddp{extra_info}.html')
    if rank == 0:
        prof.export_memory_timeline(html_export_path, device="cuda:0")
...
def main():
    ...
    for version in versions:
        train_model(version, args.type, world_size, args.sharded)
